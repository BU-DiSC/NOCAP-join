...
buff_list = [int(2**(x/2+9)) if x%2 == 0 else int((2**(x//2 + 9) + 2**(x//2 + 8))) for x in range(9)]
F = 1.02
tries = 3
...
for scale_ratio in scale_ratio_list:
    ...
    suffix = "-nosyncio.txt"
    if sys.argv[1] != 'skewed':
        output(result, 'tpch-q12-4X-lineitem-all-year-all-shipmode-exp-emul-scaling-' + str(scale_ratio) + suffix, math.ceil(scale_ratio*R*1/num_entries_per_page)) 
    else:
        output(result, 'tpch-q12-4X-lineitem-all-year-all-shipmode-exp-emul-skewed-scaling-' + str(scale_ratio) + suffix, math.ceil(scale_ratio*R*1/num_entries_per_page)) 
