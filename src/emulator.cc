...
void Emulator::get_emulated_cost_NBJ(std::string left_file_name, std::string right_file_name, uint32_t left_num_entries, uint32_t right_num_entries, bool hash){
	...
	uint32_t inner_relation_pages = max(min((params_.B - 1 - params_.NBJ_outer_rel_buffer), (uint32_t)ceil(left_num_entries*params_.left_E_size*1.0*DB_PAGE_SIZE/FUDGE_FACTOR)) + 1, 1U);
    ...
    while(true){
        ...
        for(auto i = 0; i < inner_relation_pages; i++){
	...
	if(hash){
	    ...
	    for(auto i = 0; i < inner_relation_pages  && i < R_num_pages_in_buff ; i++){
...
double selection_ratio, uint64_t* selection_seed, std::string prefix, uint32_t depth, bool build_in_mem_partition_flag){
    std::mt19937 selection_generator (*selection_seed); 
	...
	auto spill_out_partition = [&] (uint32_t subpart_idx_to_be_evicted, uint32_t random_in_memory_part_idx_to_be_evicted) {
		...
		while(j < in_memory_entries[random_in_memory_part_idx_to_be_evicted].size()) {
			...
			random_in_mem_partitions_spilled_out_flags[subpart_idx_to_be_evicted] = true;
		}
	};
    while(true){
	...
	if (build_in_mem_partition_flag && num_random_in_memory_entries < upper_bound_of_num_random_in_memory_entries) {
		...
		for (i = 0; i < num_random_in_mem_partitions; i++) {
			...
			for (const std::string s: in_memory_entries[i]) {
				key2Rvalue[s.substr(0, params_.K)] = s;
			}
...
void Emulator::get_emulated_cost_GHJ(std::string left_file_name, std::string right_file_name, uint32_t left_num_entries, uint32_t right_num_entries, uint32_t depth){
	...
	if(num_passes_R < (2 +params_.seqwrite_seqread_ratio)*params_.num_partitions && num_passes_R != 0 && ceil(left_num_entries*left_selection_ratio/params_.num_partitions/step_size/params_.hashtable_fulfilling_percent) - ceil(left_num_entries*left_selection_ratio/params_.num_partitions/step_size) >= 1){
	   //  do not use round hash if the filling percentage has go beyond the threshold
        ...
        partition_file(counter_R, {}, {}, key2Rvalue, 0, 0, left_file_name, params_.left_E_size, left_num_entries, num_passes_R, left_selection_ratio, &left_selection_seed, "part_rel_R/", depth); 
        partition_file(counter_S, {}, {}, key2Rvalue, 0, 0, right_file_name, params_.right_E_size, right_num_entries, num_passes_R, right_selection_ratio, &right_selection_seed, "part_rel_S/", depth); 
    }else{
        partition_file(counter_R, {}, {}, key2Rvalue, 0, 0, left_file_name, params_.left_E_size, left_num_entries, 0, left_selection_ratio, &left_selection_seed, "part_rel_R/", depth); 
        partition_file(counter_S, {}, {}, key2Rvalue, 0, 0, right_file_name, params_.right_E_size, right_num_entries, 0, right_selection_ratio, &right_selection_seed, "part_rel_S/", depth); 
    }
...
void Emulator::get_emulated_cost_DHH(std::string left_file_name, std::string right_file_name, uint32_t depth){
	...
	if (params_.k > 0) {
		...
		if (summed_matches*1.0/(params_.right_table_size*params_.right_selection_ratio) >= params_.DHH_skew_partition_percent) {
		    ...
		    while (ceil(R_in_pages*FUDGE_FACTOR*1.0/params_.num_partitions) + 1 > params_.B) {
		        ...
		        params_.num_partitions = std::min(params_.B - 2, params_.num_partitions);
    ...
    while(true){
		...
		for(auto i = 0; i < left_entries_per_page && read_entries < params_.left_table_size; i++){
			...
			read_entries++;
			if(params_.left_selection_ratio < 1.0 && selection_dist(selection_generator_R) >= params_.left_selection_ratio) continue;
	...
	for(auto j = 0; j < right_entries_per_page && read_entries < params_.right_table_size; j++){
		...
		read_entries++;
		if(params_.right_selection_ratio < 1.0 && selection_dist(selection_generator_S) >= params_.right_selection_ratio) continue;
            hash_value = get_hash_value(tmp_str, tmp_ht, s_seed);
	    subpartition_idx = hash_value%(params_.num_partitions);
...
uint32_t Emulator::est_best_num_partitions(uint32_t & num_of_in_memory_partitions, uint32_t & num_of_random_in_memory_entries, double a, double b, double c) {
	...
	if (delta < 0 || t == 0) {
		...
		tmp_num_total_partitions = std::min(tmp_num_total_partitions, (uint32_t)(b-1));
...
inline uint64_t overest_probe_cost_use_chernoff_bound(double floated_num_passes, double entries_from_R, double entries_from_S, uint64_t step_size) {
	...
	double upper_bound_overflowed_partition_percentage = pow(exp(delta)/pow(delta+1, delta+1), floated_num_passes*step_size);
...
uint64_t Emulator::est_probe_cost(uint32_t & num_of_in_memory_partitions, uint32_t & num_of_random_in_memory_entries, uint32_t m_r, uint32_t entries_from_S, uint32_t entries_from_R, uint32_t step_size, bool one_page_used_for_hybrid_join, const Params & params){
	...
	if (params.hybrid) {
		...
		est_best_num_partitions(num_of_in_memory_partitions, num_of_random_in_memory_entries,  (FUDGE_FACTOR*params.left_E_size)*1.0/DB_PAGE_SIZE, m_r,entries_from_R*params.left_selection_ratio);
...
uint64_t Emulator::cal_cost(uint32_t start_idx, uint32_t end_idx, std::vector<uint32_t> & SumSoFar) {
	...
	uint64_t selected_entries_from_S = static_cast<uint64_t>((SumSoFar[tmp_end_idx] - SumSoFar[tmp_start_idx])*params_.right_selection_ratio);
	uint64_t selected_entries_from_R = end_idx - start_idx+1;
	double num_passes = ceil(selected_entries_from_R*1.0/step_size);
	if (num_passes > 2 + params_.randwrite_seqread_ratio) {
		return est_GHJ_cost(selected_entries_from_R, selected_entries_from_S);
	} else if (num_passes > 2 + params_.seqwrite_seqread_ratio) {
		// Applying SMJ
		num_passes = (1 + params_.seqwrite_seqread_ratio)*ceil(log(selected_entries_from_S/(params_.B*floor(DB_PAGE_SIZE*1.0/params_.right_E_size)))/log(params_.B - 1)) + 1;
		return (uint64_t)((selected_entries_from_S + selected_entries_from_R)*num_passes);
	} else {
...
std::pair<uint32_t, uint32_t> Emulator::get_partitioned_keys(std::vector<std::string> & keys, std::vector<uint32_t> & key_multiplicity, std::unordered_map<std::string, uint16_t> & partitioned_keys, std::unordered_set<std::string> & in_memory_keys, bool & turn_on_NBJ, bool appr_flag){
    ...
    uint32_t hash_map_size = get_hash_map_size(params_.k, params_.K);
    uint32_t m_r = 0;
    ...
    uint32_t m = std::min(params_.num_partitions, (uint32_t)ceil(n*1.0/step_size)); // dynamic programming only operates when m <= n/step_size
    if(appr_flag){
	...
	uint64_t nbj_cost = ((uint64_t)ceil(params_.left_table_size*params_.left_selection_ratio/step_size)-1)*ceil(params_.right_table_size/right_entries_per_page);
	...
	for(uint32_t pages_for_skew_keys_partition = 0; pages_for_skew_keys_partition <= max_pages_for_skew_keys_partition; pages_for_skew_keys_partition++) {
		...
		if (!appr_flag) {
			...
			tmp_probe_cost = ceil(get_probe_cost(tmp_num_partitions, n, (uint32_t)round(num_in_mem_skew_entries*1.0/params_.left_selection_ratio),
			...
			while(exact_pos_k < n - num_in_mem_skew_entries) {
				...
				tmpk_hash_map_size = get_hash_map_size(exact_pos_k + num_in_mem_skew_entries, params_.K);
				...
				for(uint32_t j = min(tmp_k, 1U); j + tmpk_hash_map_size + 1 <= remaining_pages && j <= tmp_k; j++){
					...
					if (tmp_num_partitions == 0 || exact_pos_k == 0) {
						...
						m_r = remaining_pages - get_hash_map_size(num_in_mem_skew_entries, params_.K);
						tmp_probe_cost = 0;
					} else {
						// change get_probe_cost to adapt exact_pos_k
						// tmp_num_partitions might change when calling get_probe_cost function
						tmp_probe_cost = ceil(get_probe_cost(tmp_num_partitions, tmp_exact_pos_k,  (uint32_t)round(num_in_mem_skew_entries*1.0/params_.left_selection_ratio), step_size, SumSoFar, params_, cut_matrix)/right_entries_per_page);
    	            	tmp_cost = partition_cost + tmp_probe_cost; 
				...
				tmp_exact_pos_k = get_max_hash_map_entries(tmpk_hash_map_size, params_.K);
				if (tmp_exact_pos_k == exact_pos_k) {
					tmp_exact_pos_k = get_max_hash_map_entries(tmpk_hash_map_size+1, params_.K);
				}
...
void Emulator::get_emulated_cost_MatrixDP(std::vector<std::string> & keys, std::vector<uint32_t> & key_multiplicity, std::vector<uint32_t> & idxes, uint32_t buffer_in_pages, std::string left_file_name, std::string right_file_name, uint32_t left_num_entries, uint32_t right_num_entries, uint32_t depth){
    ...
    partition_file(counter_R, partitioned_keys, in_memory_keys, key2Rvalue, num_partitions, 0, left_file_name, params_.left_E_size, params_.left_table_size, 0, params_.left_selection_ratio, &left_selection_seed, "part_rel_R/", depth, true); 

    partition_file(counter_S, partitioned_keys, in_memory_keys, key2Rvalue, num_partitions, 0, right_file_name, params_.right_E_size, params_.right_table_size, 0, params_.right_selection_ratio, &right_selection_seed, "part_rel_S/", depth, false); 
...
void Emulator::get_emulated_cost_ApprMatrixDP(std::vector<std::string> & keys, std::vector<uint32_t> & key_multiplicity, std::vector<uint32_t> & idxes, uint32_t buffer_in_pages, std::string left_file_name, std::string right_file_name, uint32_t left_num_entries, uint32_t right_num_entries, uint32_t depth){
	...
	params_.num_partitions = params_.num_partitions - ceil(in_memory_keys.size()*params_.left_E_size*FUDGE_FACTOR/DB_PAGE_SIZE) - get_hash_map_size(in_memory_keys.size(), params_.K, 0) 
	 - get_hash_map_size(partitioned_keys.size(), params_.K, 0);
    ...
    if(rounded_hash){
        ...
        partition_file(counter_R, partitioned_keys, in_memory_keys, key2Rvalue, num_pre_partitions, num_random_in_mem_partitions, left_file_name, params_.left_E_size, params_.left_table_size, num_passes_left_entries, params_.left_selection_ratio, &left_selection_seed, "part_rel_R/", depth, true); 
        partition_file(counter_S, partitioned_keys, in_memory_keys, key2Rvalue, num_pre_partitions, num_random_in_mem_partitions, right_file_name, params_.right_E_size, params_.right_table_size, num_passes_left_entries, params_.right_selection_ratio, &right_selection_seed, "part_rel_S/", depth, false); 
    }else{
        partition_file(counter_R, partitioned_keys, in_memory_keys, key2Rvalue, num_pre_partitions, num_random_in_mem_partitions, left_file_name, params_.left_E_size, params_.left_table_size, 0, params_.left_selection_ratio, &left_selection_seed, "part_rel_R/", depth, true); 
        partition_file(counter_S, partitioned_keys, in_memory_keys, key2Rvalue, num_pre_partitions, num_random_in_mem_partitions, right_file_name, params_.right_E_size, params_.right_table_size, 0, params_.right_selection_ratio, &right_selection_seed, "part_rel_S/", depth, false); 
    }
